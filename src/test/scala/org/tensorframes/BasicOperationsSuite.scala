...
abstract class CommonOperationsSuite[T](implicit ev1: Numeric[T], ev2: TypeTag[T])
  extends FunSuite with TensorFramesTestSparkContext with Logging {
  import Shape.Unknown

  private val ops = new DebugRowOps
  private lazy val sql = sqlContext
  private lazy val dataOps: ScalarTypeOperation[_] = SupportedOperations.getOps[T]

  def convert(x: Double): T

  implicit class ElemConverter(x: Double) {
    def v: T = convert(x)
  }

  implicit class SeqConverter[X](xs: Seq[X]) {
    def v: Seq[Any] = xs.map {
      case x: Double => convert(x)
      case y: Any => y
    }
  }
  ...
  implicit class RowConverter(r: Row) {
    def v: Row = {
      val vs = (0 until r.length).map { i =>
        r.get(i) match {
          case u: Double => convert(u)
          case s: Seq[_] => s.v
          case x: Any => x
        }
      }
