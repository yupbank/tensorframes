...
def run_one_step2(dataframe, start_centers):
    ...
    with tf.Graph().as_default() as g:
        ...
        df2 = tfs.map_blocks([agg_points, agg_counts, agg_distances],
                             dataframe, trim=True)
    # No need to regroup by centroid, this is all done in one pass.
    with tf.Graph().as_default() as g:
        # Look at the documentation of tfs.aggregate for the naming conventions of the placeholders.
        x_input = tf.placeholder(tf.double, shape=[None, num_centroids, num_features],
...
data = RandomRDDs.normalVectorRDD(
        ...
        numRows=10, # The number of points
